...
import { Wifi, Battery, Activity, MapPin, Power, RefreshCw, Crosshair } from 'lucide-react';
...
export const DeviceMockup = () => {
    ...
    const interval = setInterval(() => {
      setProgress(prev => {
        if (prev >= 100) {
          ...
          setTimeout(() => {
             setStatus('result');
             setResultDistance(40 + Math.random() * 5); // Mock result
          }, 1500);
          return 100;
        }
        return prev + 2;
  ...
  return (
    <div className="relative mx-auto max-w-[320px] transform hover:scale-[1.02] transition-transform duration-500">
      {/* Device Body */}
                ...
                {status === 'result' && (
                    <div className="text-center space-y-4 animate-in zoom-in duration-300">
                        ...
                        <h2 className="text-5xl font-mono font-bold text-white tracking-tighter shadow-green-500/20 drop-shadow-lg">
                            {resultDistance.toFixed(1)}<span className="text-xl text-slate-500 ml-1">m</span>
                        </h2>
                        </div>

                        <div className="bg-blue-500/10 rounded-lg p-3 w-full border border-blue-500/20 animate-pulse-slow">
                        ...
                        <p className="text-xs text-slate-300 font-mono">
                            {isRTL ? 'قطاع ب، وحدة 4' : 'Sector B, Unit 4'}
                        </p>
                        </div>
                    </div>
                )}
